        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center py-sm-0">
                    ...
                    <div class="d-flex">
                        <?php if ($this->User->hasPermission('LeadSources', 'add')): ?>
                            <?= $this->Html->link(
                        ...
                        <?php if ($this->User->hasPermission('LeadSources', 'delete')): ?>
                            ...
                            <tbody>
                                <?php foreach ($leadSources as $source): ?>
                                    <tr>
                                        <td>
                                            <?php if ($this->User->hasPermission('LeadSources', 'delete')): ?>
                                                <input type="checkbox" class="select_checkbox form-check-input form-check-input-secondary" value="<?= $source->id ?>">
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <?= $source->title; ?>
                                        ...
                                        <td class="actions text-center">
                                            <?php if ($this->User->hasPermission('LeadSources', 'add')): ?>
                                                <?= $this->Html->link(
                                                    ...
                                                    '#',
                                                    [
                                                        'class' => 'text-secondary edit-lead-source',
                                                        'data-id' => $source->id,
                                            ...
                                            <?php if ($this->User->hasPermission('LeadSources', 'delete')): ?>
                                                <?= $this->Html->link(
                                                    ...
                                                    [
                                                        ...
                                                        'title' => 'Delete',
                                                        'data-record-id' => $source->id,
                                                        'data-url' => $this->Url->build(['action' => 'delete', $source->id]),
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addNewModalLabel">Add Lead Source</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?= $this->Form->create(null, ['id' => 'addNewForm', 'type' => 'post']) ?>

                <?= $this->Form->hidden('id', ['id' => 'leadSourceId']) ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on('click', '.edit-lead-source', function() {
            ...
            $('#addNewModalLabel').text('Edit Lead Source');

            $('#leadSourceId').val(id);
            $('#leadSourceTitle').val(title);
        });

        $(document).on('click', '[data-bs-target="#addNewModal"]', function() {
            if (!$(this).hasClass('edit-lead-source')) {
                $('#addNewModalLabel').text('Add Lead Source');
                $('#addNewForm')[0].reset();
                $('#leadSourceId').val('');
...
<script>
    $(document).ready(function() {
        ...
        $('#saveNewData').on('click', function() {
            ...
            var titleField = $('#leadSourceTitle');
            ...
            if (isValid) {
                ...
                $.ajax({
                    url: '<?= $this->Url->build(['controller' => 'LeadSources', 'action' => 'add']) ?>',
                    type: 'POST',
                    data: formData,

 ...
 * @var iterable<\Cake\Datasource\EntityInterface> $projectStatuses
 */
?>
<style>
    .color-tag {
        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center py-sm-0">
                    ...
                    <div class="d-flex">
                        <?php if ($this->User->hasPermission('ProjectStatuses', 'add')): ?>
                            <?= $this->Html->link(
                        ...
                        <?php if ($this->User->hasPermission('ProjectStatuses', 'delete')): ?>
                                        ...
                                        <td class="actions text-center">
                                            <?php if ($this->User->hasPermission('ProjectStatuses', 'add')): ?>
                                                <?= $this->Html->link(
                                                    ...
                                                    '#',
                                                    [
                                                        'class' => 'text-secondary edit-project-status',
                                                        'data-id' => $status->id,
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addNewModalLabel">Add Project Status</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?= $this->Form->create(null, ['id' => 'addNewForm', 'type' => 'post']) ?>

                <?= $this->Form->hidden('id', ['id' => 'projectStatusId']) ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-project-status', function() {
            var id = $(this).data('id');
            var title = $(this).data('title');
            var color = $(this).data('color');

            $('#addNewModalLabel').text('Edit Project Status');

            $('#projectStatusId').val(id);
            $('#projectStatusTitle').val(title);
        ...
        $(document).on('click', '[data-bs-target="#addNewModal"]', function() {
            if (!$(this).hasClass('edit-project-status')) {
                $('#addNewModalLabel').text('Add Project Status');
                $('#addNewForm')[0].reset();
                $('#projectStatusId').val('');
                $(".color-palet .active").removeClass("active");
...
<script>
    $(document).ready(function() {
        ...
        $('#saveNewData').on('click', function() {
            ...
            var titleField = $('#projectStatusTitle');
            ...
            if (isValid) {
                ...
                $.ajax({
                    url: '<?= $this->Url->build(['controller' => 'ProjectStatuses', 'action' => 'add']) ?>',
                    type: 'POST',
                    data: formData,

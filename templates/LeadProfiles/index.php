        ...
        <div class="col-lg-10">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center py-sm-0">
                    ...
                    <div class="d-flex">
                        <?php if ($this->User->hasPermission('LeadProfiles', 'add')): ?>
                            <?= $this->Html->link(
                        ...
                        <?php if ($this->User->hasPermission('LeadProfiles', 'delete')): ?>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table datatable-basic table-bordered table-striped">
                            ...
                            <tbody>
                                <?php foreach ($leadProfiles as $profile): ?>
                                    <tr>
                                        <td>
                                            <?php if ($this->User->hasPermission('LeadProfiles', 'delete')): ?>
                                                <input type="checkbox" class="select_checkbox form-check-input form-check-input-secondary" value="<?= $profile->id ?>">
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <?= $profile->title; ?>
                                        ...
                                        <td class="actions text-center">
                                            <?php if ($this->User->hasPermission('LeadProfiles', 'add')): ?>
                                                <?= $this->Html->link(
                                                    ...
                                                    '#',
                                                    [
                                                        'class' => 'text-secondary edit-lead-profile',
                                                        'data-id' => $profile->id,
                                            ...
                                            <?php if ($this->User->hasPermission('LeadProfiles', 'delete')): ?>
                                                <?= $this->Html->link(
                                                    ...
                                                    [
                                                        ...
                                                        'data-url' => $this->Url->build(['action' => 'delete', $profile->id]),
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addNewModalLabel">Add Lead Profile</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?= $this->Form->create(null, ['id' => 'addNewForm', 'type' => 'post']) ?>

                <?= $this->Form->hidden('id', ['id' => 'leadProfileId']) ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on('click', '.edit-lead-profile', function() {
            var id = $(this).data('id');
            var title = $(this).data('title');

            $('#addNewModalLabel').text('Edit Lead Profile');

            $('#leadProfileId').val(id);
            $('#leadProfileTitle').val(title);
        });

        $(document).on('click', '[data-bs-target="#addNewModal"]', function() {
            if (!$(this).hasClass('edit-lead-profile')) {
                $('#addNewModalLabel').text('Add Lead Profile');
                $('#addNewForm')[0].reset();
                $('#leadProfileId').val('');
...
<script>
    $(document).ready(function() {
        ...
        $('#saveNewData').on('click', function() {
            ...
            var titleField = $('#leadProfileTitle');
            ...
            if (isValid) {
                ...
                $.ajax({
                    url: '<?= $this->Url->build(['controller' => 'LeadProfiles', 'action' => 'add']) ?>',
                    type: 'POST',
                    data: formData,
